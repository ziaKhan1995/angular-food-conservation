...
const getNewSaleTickeEmailTemplate = async (ticket) => {
    ...
    //     throw new IncompleteTicketError('customer null');
    // }
    [count, resultTicketLines] = await TicketlinesController.getTicketlinesByTicketId(ticket.id);
    if (!resultTicketLines || !resultTicketLines.count) {
        console.info('********no ticketlines found');
        throw new IncompleteTicketError('ticketlines null');
    }
    ...
    var emailTemplates = await EmailTemplatesController.getEmailTemplatesByMediaPurposeId(
        EMAIL_TEMPLATE_PURPOSE_TYPES.EMAIL_SALE_TO_BUSINESS_OWNER.code);
    if (!emailTemplates) {
    ...
    var saleRows = "";
    var rowsTciketLines = resultTicketLines.rows;
    for (let i = 0; i < resultTicketLines.count; i++) {
        var ticketLine = rowsTciketLines[i];
        // console.info('ticketLine',ticketLine);
        var porperties = ticketLine.properties.toString('utf8');
        var price = ticketLine.price + ' ' + currency
        var productName = "";
        try {
            var startIndex = porperties.indexOf("<entry key=\"product.name\">") + 26;
            var subString = porperties.substring(startIndex);
            var endIndex = subString.indexOf("</entry>");
            productName = subString.substring(0, endIndex);
            // console.info('productName=', productName);
        } catch (err) {
            console.error('err=' + err.stack);
        }

        saleRows = saleRows + "  <tr class='service'>" + "  <td class='tableitem'><p class='itemtext'>"
            + productName + "</p></td>" + "  <td class='tableitem'><p class='itemtext'>"
            + ticketLine.units + "</p></td>" + " <td class='tableitem'><p class='itemtext'>" + price
            + "</p></td>" + " </tr>";
    }
    template = template.replace("$$customersalerows$$", saleRows);
    var paymentRows = "";
    var totalAmount = 0.0;
...
module.exports.getNewSaleTickeEmailTemplate = getNewSaleTickeEmailTemplate;
